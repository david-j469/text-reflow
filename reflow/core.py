...
def _split_block(raw_lines):
    """Split one blank-line-delimited block into prose strings and list blocks.
...
def normalize_paragraphs(text):
    ...
    Each element is either a plain string (an ordinary paragraph) or a list
    of (marker, text) tuples (a run of bullet/numbered list items, kept
    separate so they don't get merged into one line).
    """
    text = text.replace("\r\n", "\n").replace("\r", "\n").strip()
    if not text:
        return []
...
def wrap_text(text, width=72):
    ...
    wrapped = [
        _wrap_list_block(p, width)
        if isinstance(p, list)
        else textwrap.fill(p, width=width, break_long_words=False, break_on_hyphens=False)
        for p in paragraphs
    ]
    return "\n\n".join(wrapped)
